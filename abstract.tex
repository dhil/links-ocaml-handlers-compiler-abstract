...
continuations. We have retrofitting the experimental, functional
programming language Links with algebraic effects and effect handlers
\cite{Hillerstrom2016}. Furthermore, Links uses row types to provide
extensible effect typing.

Links was originally conceived as a single language for multi-tier
web-programming \cite{Cooper2006}. The Links infrastructure is
comprised of three backends:
\begin{enumerate*}[label={\roman*)}]
 \item JavaScript based client-side,
 \item interpreted Links server-side,
 \item and a SQL database backend.
\end{enumerate*}
We have implemented a compiler to complement to the server-side
backend. Our compiler interfaces directly with the OCaml Multicore
backend \cite{Dolan2015}. By taking advantage of the OCaml backend we
obtain both native code compilation support and the OCaml toolchain
for free.
