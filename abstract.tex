...
the OCaml \emph{Lambda} language, which is a small, untyped lambda
calculus. The OCaml backend exposes a hierarchy of intermediate
representations (IRs), where the top representation is known as
Lambda. As shown in the Figure \ref{fig:infra-diagram} the Lambda IR
offers two different compilation options: byte code and native code.
Therefore by targetting Lambda rather than a lower level IR, we
achieve maximum flexibility as a translation into byte code, in
principle, enables us to take advantage of custom backends such as
...
amounts to a lot of value boxing. Thus, we target Lambda for greater
flexibility and control.  We effectively subvert OCaml's typechecker
by targetting Lambda, however the translation is safe as Links
programs are already typechecked.
...
structures as a consequence composition of handlers gives rise to
$n$-element stacks at run-time. For example, the composition
\lstinline[mathescape]!randomResult(maybeResult($\cdot$))! gives rise
to a two-element stack. OCaml provides only affine handlers. Thus, an
invocation of an abstract operation amounts to a dynamic lookup for a
suitable handler in a stack.

Furthermore, OCaml supports only affine handlers, because OCaml
implements only efficient, one-shot continuations. Multiple
invocations of a continuation causes a run-time error. Although,
multi-shot handlers can be simulated by manually cloning continuations
using \lstinline$Obj.clone$. However, this provide a fragile
abstraction, because if the handler stack contains at least one
multi-shot handler, then every affine handler in the stack most be
demoted to a multi-shot handler. The reason for this is that a
multi-shot handler may consume a linear continuation more than
once. Consequently, multi-shot handlers in OCaml breaks modularity.

In the Links compiler we use the cloning capability under the hood to
implement multi-shot handlers. For example, our encoding of
\lstinline$allResults$ amounts to the following in plain OCaml:
...
let all_results m = match m () with
  | x -> [x]
  | effect Choose k -> 
      let k' arg = continue (Obj.clone k) arg in
      k' true @ k' false
\end{lstlisting}
OCaml provides a unified syntax for pattern-matching on regular,
effect, and exception patterns. The keyword
\lstinline[style=ocaml]$effect$ begins an
operation-clause. Essentially, we create a local function
\lstinline$k'$, which wraps the actual continuation \lstinline$k$. An
invocation of \lstinline$k'$ passes its argument to a fresh copy of
the actual continuation. The \lstinline$continue$ function is provided
by the standard library; given a continuation and a value, it invokes
the continuation with that particular value. By default we implement
every handler as a multi-shot handler.
...
linear handlers at compile time.  We plan to use Links' linear type
system to track the linearity of handlers.

\paragraph{Handler resolution} 
Traversing a large handler stack is likely to be costly. If the
handler stack, or part of it, is known at statically, then we can
instantiate abstract operations at compile time.

\paragraph{Linear effects} \dots
\dhil{Let programmers annotate effects with linearity to give performance guarantees.}
